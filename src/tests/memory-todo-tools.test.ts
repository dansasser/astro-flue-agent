...
test('todo tools create/complete/list through the trusted event scope', async () => {
  ...
  try {
    const created = JSON.parse(
      await runTool(createTodoTool, { eventId: event.id, title: 'Run smoke', priority: 'high' }),
    ...
    const completed = JSON.parse(
      await runTool(completeTodoTool, { eventId: event.id, id: created.todo.id }),
    ...
    const active = JSON.parse(await runTool(listTodosTool, { eventId: event.id })) as {
      todos: { id: string }[];
    };
...
test('update_todo changes priority', async () => {
  ...
  try {
    const created = JSON.parse(
      await runTool(createTodoTool, { eventId: event.id, title: 'T' }),
    ) as { todo: { id: string } };
    const updated = JSON.parse(
      await runTool(updateTodoTool, { eventId: event.id, id: created.todo.id, priority: 'urgent' }),
