...
Trusted file execution uses Flue's Node local sandbox factory through the worker-owned tool/runtime layer. Shell, Git, and test child processes run inside a Bubblewrap mount and process namespace that exposes the configured runtime workspace read-write, the active Node and system runtime read-only, and a private temporary directory. Owner runtime files outside the workspace are not mounted. An authenticated private Git child may additionally receive one secret-free read-only askpass helper file and its command-scoped credential environment. Process execution fails closed if that Linux isolation boundary is unavailable. Task execution scopes to either the workspace root itself or a selected project/repo under \`projects/**\` or \`repos/**\`. The main orchestrator does not own these execution tools.
