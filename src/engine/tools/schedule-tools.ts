...
import { getScheduleManager } from '../../engine/schedules/boot.js';
import { scheduleInstanceId } from '../../engine/schedules/schedule-types.js';
import { loadOwnedSchedule } from '../../engine/schedules/schedule-ownership.js';
import { emitScheduleProgress } from '../../engine/schedules/schedule-telemetry.js';
import { getTrustedMemoryEvent } from '../../engine/tools/memory-tools-shared.js';
...
export const scheduleCreateTool = defineTool({
  ...
  run: async ({ data: { eventId, slug, kind, schedule, prompt, targetAgent, tz, payload, deleteAfterRun } }) => {
...
export const scheduleUpdateTool = defineTool({
  ...
  run: async ({ data: { eventId, slug, schedule, prompt, payload, tz, enabled } }) => {
    const manager = requireManager();
    const ownerScope = deriveOwnerScope(String(eventId));
    const owned = loadOwnedSchedule(manager.store, String(slug), ownerScope);
    if (!owned.ok) {
...
export const scheduleRunsTool = defineTool({
  ...
  run: async ({ data: { eventId, slug, limit } }) => {
    const manager = requireManager();
    const ownerScope = deriveOwnerScope(String(eventId));
    const owned = loadOwnedSchedule(manager.store, String(slug), ownerScope);
    if (!owned.ok) {
