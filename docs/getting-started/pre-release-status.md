...
| `REL-CW-003` | Complete fail-closed filesystem and shell containment with allow-once/session escalation | A partial absolute-path regex exists outside the approved architecture and does not cover the operation surface | `implement-file-access-approval-gate` | Required |
